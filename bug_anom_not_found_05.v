...
| Array (instance, constr array, constr, constr)
  ...
  Ltac2 equal_modulo_univs (x : constr) (y : constr) : bool :=
    if Constr.equal_nounivs x y
    then if Constr.equal x y
         then
    let dummy := mkRel (-1) in
    if Constr.equal x y
    then true
    else (printf "Checking: %t ≟ %t" x y;
          match! mkApp dummy [x; y] with
          | ?f ?x ?x => printf "Success"; true
          | _ => printf "Not equal"; false
          end).
End Constr.
    ...
    Module Export Cache.
      ...
      Ltac2 find_opt (term : constr) (cache : t) : elem option
        := let (_, cache) := cache.(contents) in
           List.assoc_opt Constr.equal_modulo_univs term cache.
      ...
      Ltac2 add (head_constant : constr) (term : constr) (rterm : constr) (cache : t) : ident
        := let (avoid, known) := cache.(contents) in
           match List.assoc_opt Constr.equal_modulo_univs term known with
             ...
             let found :=
               ...
               | Constr.Unsafe.App c args
                 => Reify.debug_enter_reify_case "expr.reify_in_context" "App (check LetIn)" term;
                    if Constr.equal_modulo_univs c '@Let_In
    ...
    Module Import Tactics.
        ...
        => let ty := Std.eval_cbv (strategy:(beta)) ty in
           ...
           let rty := match! all_base_and_interp with
                      | context[Datatypes.cons (?rty, ?ty')]
                        => if Constr.equal_modulo_univs ty ty'
                           then Some rty
           ...
           | None
             =>
               match! ty with
               | ?base_interp' ?t
                 => if Constr.equal_modulo_univs base_interp' base_interp
                    then Some t
                    else Control.zero Match_failure
               | @base.interp ?base' ?base_interp' (@base.type.type_base ?base' ?t)
                 => if Constr.equal_modulo_univs base_interp' base_interp && Constr.equal_modulo_univs base base
                    then Some t
                    else Control.zero Match_failure
      ...
      Ltac2 rec is_recursively_constructor_or_literal (term : constr) : bool :=
        ...
        | Constr.Unsafe.App f args
          => if Constr.equal_modulo_univs f '@ident.literal
             then true
             else
      ...
      Ltac2 reify_ident_from_interp (ident_interp : constr) (term : constr) : constr option :=
        ...
        lazy_match! term with
        | ?ident_interp' _ ?idc
          => if Constr.equal_modulo_univs ident_interp ident_interp'
             then Some idc
             else None
        | _ => None
        end.
        ...
        let ident_Literal := let idc := '(@ident.literal) in
                             let found := match! all_ident_and_interp with
                                          | context[GallinaAndReifiedIdentList.cons ?ridc ?idc']
                                            => if Constr.equal_modulo_univs idc idc'
                                               then Some ridc
                                               else Control.zero Match_failure
                                          | _ => None
                                          end in
